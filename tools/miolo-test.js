...
t('json tem 24 frases', frases.length === 24);
...
frases.forEach(function (f) {
  ...
  if (f.type !== 'citação') tipoOk = false;
...
t('todas são citações (type="citação")', tipoOk);
...
t('ids seguem a ordem das datas', frases.every(f => f.id >= 1 && f.id <= 24) && frases.every((f, i) => i + 1 === f.id && f.date === datas[i]));
...
t('Química: 3', conta['Química'] === 3);
t('Física: 8', conta['Física'] === 8);
t('Biologia: 5', conta['Biologia'] === 5);
t('Matemática: 5', conta['Matemática'] === 5);
t('Astronomia: 2', conta['Astronomia'] === 2);
t('Ciências da Terra: 0', !conta['Ciências da Terra']);
t('Ciência dos Materiais: 0', !conta['Ciência dos Materiais']);
t('Computação: 1', conta['Computação'] === 1);
t('soma 24', Object.values(conta).reduce((a, b) => a + b, 0) === 24);

console.log('== fallback embutido ==');
t('fallback tem 24 frases', Fallback.frases.length === 24);
